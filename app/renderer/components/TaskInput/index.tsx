...
export default class TaskInput extends React.PureComponent<InputProps, InputState> {
  ...
  renderBefore() {
    ...
    return !!addonBefore && <div styleName="task-input-center">{addonBefore}</div>;
  }

  renderAfter() {
    const { addonAfter } = this.props;
    return !!addonAfter && <div styleName="task-input-center">{addonAfter}</div>;
  }

  renderClear() {
    const { allowClear } = this.props;
    return (
      !!allowClear && this.state.text && <i styleName="task-input-clear" onClick={this.onClear} />
    );
  }
  ...
  renderInput() {
    ...
    return (
      <div
        styleName={classnames(`task-input-input`, {
          [`${size}`]: true,
        })}
      >
        <input
  ...
  renderTextarea() {
    ...
    return (
      <div styleName="task-input-textarea" style={{ height: 24 * _rows }}>
        <textarea
          {...{ placeholder, maxLength, id, disabled, autoFocus }}
          rows={_rows}
          value={text}
        ...
        {this.renderClear()}
        {allowCount && (
          <div styleName="task-input-textarea-footer">
            <span
  ...
  render() {
    ...
    return (
      <div
        style={style}
        styleName={classnames('task-input', {
